...
secure_asset('css/core.css?'.time())}}" type="text/css"/>
    <link rel="stylesheet" href="{{ 
secure_asset('libs/inc/TimeCircles.css')}}" type="text/css"/>
...
initial-scale=1.0">
    ...
    <script src="{{ 
secure_asset('/libs/inc/TimeCircles.js')}}"></script>
    ...
    </div> </header> <body id="home"> <div id="page">
    <div class="container">
        <div class="currency-selection-block">
            <h2 class="h2">Choose currency</h2>
            <ul class="curriencies-list">
                @foreach ($payment_methods as $payment_method)
                    <li class="coin-icon coin-icon-34 
coin-rounded-{{$payment_method->name}}">
                        <a 
href="{{url('invoice/'.$invoice->hash.'/'.$payment_method->short_code)}}">{{$payment_method->name}}</a>
                    </li>
                @endforeach
            </ul> <br> <br> <center>Gift Card</center>
        </div>
    </div> </div> <footer>
